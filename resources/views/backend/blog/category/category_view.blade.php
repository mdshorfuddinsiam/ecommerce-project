			...
			<div class="col-8">
				 <div class="box">
					<div class="box-header with-border">
					  <h3 class="box-title">All Blog Category<span class="badge badge-pill badge-success">{{ count($blogCategories) }}</span></h3>
					</div>
					<!-- /.box-header -->
					<div class="box-body">
						<div class="table-responsive">
						  <table id="example1" class="table table-bordered table-striped">
							...
							<tbody>
								...
								<tr>
									<td>{{ @$row->blog_category_name_en }}</td>
									<td>{{ @$row->blog_category_name_bn }}</td>
									<td>{{ @$row->status == '1' ? 'Active' : 'Inactive' }}</td>
									<td>
										<a href="{{ route('blog.category.edit', @$row->id) }}" class="btn btn-info mr-5"><i class="fa fa-pencil"></i></a>
										<a href="{{ route('blog.category.delete', @$row->id) }}" id="delete" class="btn btn-danger"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
								@endforeach
							</tbody>
